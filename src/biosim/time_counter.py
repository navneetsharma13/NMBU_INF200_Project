...
class Plot:
    ...
    def init_plot(self,car_num,year_num):
        template = 'Year: {:5d}'
        self.txt = self.axt.text(0.5, 0.5, template.format(0),
        ...
        max_step = 300
        step_size = 1
        self.ax2.set_title('Carnivore/Herbivore Pop:')
        self.ax2.set_xlim(0, max_step)
        self.ax2.set_ylim(0, 100)
        xdata = np.arange(0, max_step, step_size)
        line = self.ax2.plot(xdata, np.full_like(xdata, np.nan, dtype=float), 'b-')[0]
        random.seed(12345)
        # for n in range(0, max_step, step_size):
        self.txt.set_text(template.format(year_num))
